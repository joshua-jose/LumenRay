...
use crate::{
    renderer::{CPURenderer, CameraComponent, PointLightComponent, TransformComponent},
    scene::Scene,
    vec3,
...
impl Engine {
    ...
    fn render(&mut self, metric_file: &mut File) {
    ...
    fn update(&mut self, n: &mut u32) {
        *n += 1;

        let light_pos = vec3(4.0 * (*n as f32 / 20.0).sin(), 2.0, -1.0);
        for (_, (light_t, _)) in self
            .scene
            .query_mut::<(&mut TransformComponent, &PointLightComponent)>()
        {
        ...
        for (_, (t, c)) in self
            .scene
            .query_mut::<(&mut TransformComponent, &mut CameraComponent)>()
        {
    ...
    pub fn run(mut self) {
        ...
        event_loop.run(move |ev, _, control_flow| match ev {
            ...
            Event::MainEventsCleared => self.on_window_update(&mut n, &mut metric_file),
    ...
    fn on_window_update(&mut self, n: &mut u32, metric_file: &mut File) {
        self.update(n);
        self.render(metric_file);
    }
}
